...
import net.lightstone.util.Perlin2D;
...
public class InfiniMapWorldGenerator implements WorldGenerator {
    ...
    abstract private class Biome {
        ...
        abstract public int get(int y, int h);

        // Block mappers

        protected int forestBlock(int y, int h) {
            if (y > h) return Blocks.TYPE_AIR;
            else if (y == h) return Blocks.TYPE_GRASS;
            else if (y > h - 5) return Blocks.TYPE_DIRT;
            else if (y <= 1) return Blocks.TYPE_BEDROCK;
            else return Blocks.TYPE_STONE;
        }

        protected int desertBlock(int y, int h) {
            if (y > h) return Blocks.TYPE_AIR;
            else if (y > h - 5) return Blocks.TYPE_SAND;
            else if (y <= 1) return Blocks.TYPE_BEDROCK;
            else return Blocks.TYPE_STONE;
        }

        protected int coastalBlock(int y, int h) {
            if (h >= 64) return forestBlock(y, h);
            else if (y > 64) return Blocks.TYPE_AIR;
            else if (y > h) return Blocks.TYPE_WATER;
            else if (y <= 1) return Blocks.TYPE_BEDROCK;
            else return Blocks.TYPE_STONE;
        }

        protected int floatingBlock(int y, int h) {
            int h2 = 128 - 16;
            int base = 128 - 16;
            int h3 = base - 3 * (h2 - base);
            if (y > h2) return Blocks.TYPE_AIR;
            else if (y == h2 && y > h3) return Blocks.TYPE_GRASS;
            else if (y == h2 && y == h3) return Blocks.TYPE_SAND;
            else if (y >= h2 - 8 && y >= h3) return Blocks.TYPE_DIRT;
            else if (y >= h3) return Blocks.TYPE_STONE;
            else if (y > 64) return Blocks.TYPE_AIR;
            else if (y > h) return Blocks.TYPE_WATER;
            else if (y >= h - 5) return Blocks.TYPE_GRAVEL;
            else if (y > 1) return Blocks.TYPE_STONE;
            else return Blocks.TYPE_BEDROCK;
        }
    }
    ...
    private class Forest extends Biome {
        ...
        public int get(int y, int h) { return coastalBlock(y, h); }
    }
    ...
    private class Floating extends Biome {
        ...
        public int get(int y, int h) { return floatingBlock(y, h); }
    }

    private class Desert extends Biome {
        public Desert() { super(20, 68, 16, 1, 2, 4); }
        public int get(int y, int h) { return desertBlock(y, h); }
    }
		...
		for (int x = 0; x < Chunk.WIDTH; x++) {
			for (int z = 0; z < Chunk.HEIGHT; z++) {
				...
				for (int y = 0; y < Chunk.DEPTH; y++) {
					chunk.setType(x, z, y, biome.get(y, height));
